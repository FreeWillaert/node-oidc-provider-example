...
const expressPromise = oidc.initialize({
  ...
  expressApp.get('/interaction/:grant', (req, res) => {
    ...
    oidc.interactionDetails(req)
      .then(details => {
        ...
        console.log("get /interaction/:grant -- start rendering...");
        res.render(view, { details });
        console.log("get /interaction/:grant -- rendering done!");
      })
      .catch(next);
  ...
  expressApp.post('/interaction/:grant/confirm', parse, (req, res) => {
    ...
    console.log("post /interaction/:grant/confirm -- all done!");
  });
  ...
  expressApp.post('/interaction/:grant/login', parse, (req, res, next) => {
    ...
    Account.authenticate(req.body.email, req.body.password)
      .then((account) => {
        ...
        console.log("post /interaction/:grant/login -- start interactionFinished...");
        oidc.interactionFinished(req, res, {
          login: {
        ...
        console.log("post /interaction/:grant/login -- interactionFinished done!");
      }).catch(next);

    console.log("get /interaction/:grant/login -- all done!");
  });
